    ...
    public class VREyeRaycaster : MonoBehaviour
    {
        ...
        private void EyeRaycast()
        {
            ...
            if (Physics.Raycast(ray, out hit, m_RayLength, ~m_ExclusionLayers))
            {
				...
				Vector2 test = new Vector2 (hit.textureCoord.x, hit.textureCoord.y);
				OnlineMaps map = OnlineMaps.instance;
				//Debug.LogError("Name: "+hit.collider.name);

				OnlineMapsTextureControl mapTextControl = hit.collider.GetComponent<OnlineMapsTextureControl> ();
				if (mapTextControl != null) {
					Vector2 geo = mapTextControl.GetCoords (test);
					//Debug.LogError(geo.x+" "+geo.y);				
				}
